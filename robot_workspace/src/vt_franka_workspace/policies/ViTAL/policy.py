...
class ACT:
    ...
    def pre_process(self, qpos):
        """Normalize input joint positions using ViTAL's shared action/qpos stats."""
    ...
    def post_process(self, action):
        """Denormalize model outputs using ViTAL's shared action/qpos stats."""
        if self.stats is not None:
            mean = (self.stats["qpos_mean"] + self.stats["action_mean"]) / 2
            std = (self.stats["qpos_std"] + self.stats["action_std"]) / 2
    ...
    def get_action(self, obs=None):
        ...
        with torch.no_grad():
            ...
            if self.query_every_step or self.t % self.query_frequency == 0:
            ...
            if self.temporal_agg:
                ...
                action_index = 0 if self.query_every_step else self.t % self.query_frequency
                raw_action = self.all_actions[:, action_index]
