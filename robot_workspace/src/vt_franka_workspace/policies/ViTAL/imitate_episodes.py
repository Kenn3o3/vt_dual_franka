            ...
            json.dump(
                {
                    ...
                    'monitor_value': float(min_val_loss),
                    'mode': 'min',
...
def train_bc(policy:ACTPolicy,
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        if epoch_val_loss < min_val_loss:
            ...
            with open(os.path.join(ckpt_dir, 'best.info.json'), 'w', encoding='utf-8') as f:
                json.dump(
                    {
                        'checkpoint': 'policy_best.ckpt',
                        'monitor_key': 'val_loss',
        ...
        with open(loss_log_path, 'a', encoding='utf-8') as f:
            json.dump(
                {
                    'epoch': int(epoch),
                    'train_loss': float(epoch_train_loss.item()),
                    'val_loss': float(epoch_val_loss.item()),
