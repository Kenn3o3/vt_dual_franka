...
class ACT:
    ...
    def __init__(self, args_override=None, RoboTwin_Config=None):
        ...
        self.temporal_agg = bool(args_override.get("temporal_agg", False))
        ...
        if ckpt_dir:
            ...
            if not ckpt_path:
                policy_best_path = os.path.join(ckpt_dir, "policy_best.ckpt")
                best_path = os.path.join(ckpt_dir, "best.ckpt")
                ckpt_path = policy_best_path if os.path.exists(policy_best_path) else best_path
    ...
    def _use_quaternion_temporal_agg(self, obs):
        qpos = obs.get("qpos")
        return self.state_dim == 8 and qpos is not None and len(qpos) >= 7

    def _aggregate_quaternion_actions(self, actions_for_curr_step, exp_weights, obs):
        action_candidates = self.post_process(actions_for_curr_step.detach().cpu().numpy())
        quat_candidates = action_candidates[:, 3:7].copy()
        ref_quat = self._normalize_quaternion(obs["qpos"][3:7])
        if ref_quat is None:
            return None

        for quat_idx in range(len(quat_candidates)):
            quat_unit = self._normalize_quaternion(quat_candidates[quat_idx])
            if quat_unit is None:
                quat_candidates[quat_idx] = 0.0
                continue
            if np.dot(quat_unit, ref_quat) < 0:
                quat_candidates[quat_idx] *= -1.0

        weights = exp_weights.detach().cpu().numpy()
        quat = (quat_candidates * weights).sum(axis=0)
    ...
    def get_action(self, obs=None):
        ...
        with torch.no_grad():
            ...
            if self.temporal_agg:
                ...
                # Use ACT temporal aggregation weighting from imitate_episodes.py.
                exp_weights = np.exp(-self.temporal_agg_k * np.arange(len(actions_for_curr_step)))
                exp_weights = exp_weights / exp_weights.sum()
                exp_weights = (torch.from_numpy(exp_weights).to(self.device).unsqueeze(dim=1))

                raw_action = (actions_for_curr_step * exp_weights).sum(dim=0, keepdim=True)
                if self._use_quaternion_temporal_agg(obs):
                    aggregated_quat = self._aggregate_quaternion_actions(actions_for_curr_step, exp_weights, obs)
            else:
        ...
        if aggregated_quat is not None:
            action[..., 3:7] = aggregated_quat
