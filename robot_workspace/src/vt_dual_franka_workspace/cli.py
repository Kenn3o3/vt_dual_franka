...
from vt_dual_franka_shared.transforms import BimanualCalibration, SingleArmCalibration
...
def main() -> None:
    ...
    make_bimanual_dataset = subparsers.add_parser("make-bimanual-dataset", help="Build the dual-arm commanded-action common dataset")
    make_bimanual_dataset.add_argument("collect_task_dir", type=Path)
    make_bimanual_dataset.add_argument("--output-dir", type=Path, default=None)
    make_bimanual_dataset.add_argument("--name", "--dataset-name", dest="dataset_name", default="real_bimanual_v1")
    make_bimanual_dataset.add_argument("--target-hz", type=float, default=10.0)
    make_bimanual_dataset.add_argument("--max-action-lead-sec", type=float, default=None)
    make_bimanual_dataset.add_argument("--overwrite", action="store_true")
    ...
    dual_teleop = subparsers.add_parser("dual-teleop", help="Run natural left/right Quest teleop for two Franka arms")
    dual_teleop.add_argument("--workspace-config", default="config/workspace.yaml")
    dual_teleop.add_argument("--poll-hz", type=float, default=60.0)
    ...
    if args.subcommand == "make-bimanual-dataset":
        ...
        print(f"Bimanual common dataset manifest: {manifest_path}", flush=True)
        return
    ...
    if args.subcommand == "dual-teleop":
        try:
            import uvicorn
        except ImportError as exc:
            raise RuntimeError("Failed to import uvicorn for dual-teleop") from exc
        ...
        calibration = BimanualCalibration.from_dir(workspace.calibration.calibration_dir)
        service = DualQuestTeleopService(workspace.teleop, coordinator, calibration)
        coordinator.start()
        service.set_teleop_enabled(True)
        try:
            print(f"Dual teleop API: http://{workspace.teleop.host}:{workspace.teleop.port}/unity", flush=True)
            uvicorn.run(create_dual_teleop_app(service), host=workspace.teleop.host, port=workspace.teleop.port)
        finally:
            service.stop()
            coordinator.stop()
        return
...
def _build_controller(workspace):
    ...
    if hasattr(controller, "arm_id"):
        controller.arm_id = "left"
    return controller
