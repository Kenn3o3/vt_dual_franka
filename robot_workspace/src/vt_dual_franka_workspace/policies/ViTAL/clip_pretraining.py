...
def clip_pretraining(train_loader: DataLoader,
    ...
    # train the model
        ...
        # keep only the best validation checkpoint on disk
        epoch_test_loss = float(testing_losses[epoch].mean())
        if epoch_test_loss < best_test_loss:
            ...
            with open(f'{save_dir}/best.info.json', 'w', encoding='utf-8') as f:
                json.dump(
                    {
                        'best_epoch': int(epoch),
                        'best_test_loss': float(best_test_loss),
                        'best_checkpoint_prefix': 'best_',
                    },
                    f,
                    indent=2,
                    sort_keys=True,
