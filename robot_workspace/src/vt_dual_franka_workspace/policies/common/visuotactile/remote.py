...
class FetchInferenceBundleConfig:
    ...
    token: str | None = None
    repo_root: Path = REPO_ROOT
    ssd_root: Path = Path("/mnt/kenny_ssd/vt_dual_franka")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sync and train a VT Dual Franka visuotactile policy on a remote PC")
...
def build_pipeline_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Publish/fetch VT Dual Franka visuotactile inference bundles")
    ...
    fetch.add_argument("--ssd-root", type=Path, default=Path("/mnt/kenny_ssd/vt_dual_franka"))
