...
This folder contains the VT Dual Franka adapter for the MPD-family baselines from `UNTOUCHED_REFERENCE/mpd/baseline_train.sh`.
...
python -m vt_dual_franka_workspace.policies.mpd.prepare \
...
```bash
python -m vt_dual_franka_workspace.policies.mpd.smooth_gripper_dataset \
  --workspace-config robot_workspace/config/workspace.yaml \
  --task-name put_cup_on_plate \
...
```bash
python -m vt_dual_franka_workspace.policies.mpd.train \
  --workspace-config robot_workspace/config/workspace.yaml \
  --task-name put_cup_on_plate \
  --algorithm dp \
  --device cuda \
