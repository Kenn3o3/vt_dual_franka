...
def split_episode_indices(num_episodes, train_ratio=0.8):
    episode_ids = np.arange(num_episodes, dtype=np.int64)
    if num_episodes <= 1:
        return episode_ids, np.array([], dtype=np.int64)

    split_idx = min(max(1, int(num_episodes * train_ratio)), num_episodes - 1)
    return episode_ids[:split_idx], episode_ids[split_idx:]
...
def load_data(dataset_dir, num_episodes, camera_names, tactile_names, batch_size_train, batch_size_val, chunk_size):
    ...
    # Use a deterministic sequential split: first 80% train, last 20% val.
    train_indices, val_indices = split_episode_indices(num_episodes, train_ratio=0.8)
