...
class ConsoleNoiseFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        message = record.getMessage()
        if (
            record.name == "vt_dual_franka_workspace.teleop.quest_server"
            and "Teleop control iteration failed: Controller state is stale:" in message
        ):
            return False
        if (
            record.name == "vt_dual_franka_workspace.publishers.state_bridge"
            and "State bridge iteration failed: Controller state is stale:" in message
        ):
            return False
        if (
            record.name == "vt_dual_franka_workspace.sensors.orbbec.recorder"
