...
def add_expand_dim(data: Union[torch.Tensor, np.ndarray],
    ...
    # Build the index tuple directly (avoids eval-based nested scoping issues
    # that break torch.compile / TorchDynamo tracing).
    index_tuple = []
    expand_sizes = []
    add_dim_index = 0
    for dim in range(num_data_dim + num_dim_to_add):
        if dim in add_dim_indices or dim in add_dim_reverse_indices:
            index_tuple.append(None)
            expand_sizes.append(add_dim_sizes[add_dim_index])
            add_dim_index += 1
        else:
            index_tuple.append(slice(None))
            if type(data) == torch.Tensor:
                expand_sizes.append(-1)
            elif type(data) == np.ndarray:
                expand_sizes.append(1)
            else:
                raise NotImplementedError

    indexed_data = data[tuple(index_tuple)]
    if type(data) == torch.Tensor:
        return indexed_data.expand(expand_sizes)
    else:
        return np.tile(indexed_data, expand_sizes)
