...
class GraspLiftTouchEnv(SofaEnv):
            ...
            if self.active_phase >= Phase.TOUCH:
                ...
                while not target_visible:
                    ...
                    if self.internal_render_mode == RenderMode.NONE:
                        ...
                        # Fixed check region size (20 pixels radius) to ensure resolution-independent behavior
                        CHECK_RADIUS = 20
                        low = self.target_position_in_image - CHECK_RADIUS
                        high = self.target_position_in_image + CHECK_RADIUS
                        # Clip to image bounds
                        low = np.maximum(low, 0)
                        high = np.minimum(high, np.asarray(image.shape[:2]))
    ...
    def _get_reward_features(self, image: np.ndarray, previous_reward_features: dict) -> dict:
        ...
        # Target visibility
        # Fixed check region size (20 pixels radius) to ensure resolution-independent behavior
        # This corresponds to 1/10 of the reference resolution (200x200)
        CHECK_RADIUS = 20
        low = self.target_position_in_image - CHECK_RADIUS
        high = self.target_position_in_image + CHECK_RADIUS
        # Clip to image bounds
