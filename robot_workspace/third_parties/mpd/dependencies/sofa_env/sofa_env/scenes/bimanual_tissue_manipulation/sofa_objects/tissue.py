...
class Tissue:
    def __init__(
        ...
        # BoxROI may return empty indices when MechanicalObject positions are not yet
        # available during _init_sim(). Fall back to pure-Python geometric filtering.
        if not self.top_right_corner_indices or not self.top_left_corner_indices:
            positions = np.array(self.grid_topology.position.array(), dtype=np.float64)

            def _box_indices(positions: np.ndarray, box_min, box_max, translation, rotation_deg_z) -> list:
                """Return indices of positions inside a rotated bounding box (2-D, z-axis rotation)."""
                t = np.array(translation[:2])
                theta = np.deg2rad(rotation_deg_z)
                c, s = np.cos(theta), np.sin(theta)
                R = np.array([[c, s], [-s, c]])  # inverse rotation to bring points into box frame
                pts_local = (positions[:, :2] - t) @ R.T
                bmin = np.array(box_min[:2])
                bmax = np.array(box_max[:2])
                mask = np.all((pts_local >= bmin) & (pts_local <= bmax), axis=1)
                return list(np.where(mask)[0])

            if not self.top_right_corner_indices:
                self.top_right_corner_indices = _box_indices(
                    positions,
                    box_min=(-2.5, -15.0, -1.5),
                    box_max=(2.5, 5.0, 1.5),
                    translation=(0.5 * self.grid_size[0], 0.5 * self.grid_size[1], 0.0),
                    rotation_deg_z=-angle,
                )
            if not self.top_left_corner_indices:
                self.top_left_corner_indices = _box_indices(
                    positions,
                    box_min=(-2.5, -15.0, -1.5),
                    box_max=(2.5, 5.0, 1.5),
                    translation=(-0.5 * self.grid_size[0], 0.5 * self.grid_size[1], 0.0),
    ...
    def rigidify(self, rigidification_indices: Union[List[int], List[List[int]]]) -> None:
        ...
        # Convert to plain Python list via numpy to ensure SOFA can parse the position data correctly
        rigid_parts.addObject("MechanicalObject", template="Rigid3d", position=np.array(subset_reference_poses, dtype=float).tolist())
