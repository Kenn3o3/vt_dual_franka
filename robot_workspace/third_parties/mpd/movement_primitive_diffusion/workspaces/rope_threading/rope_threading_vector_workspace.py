...
class RopeThreadingEnvVectorWorkspace(BaseVectorWorkspace):
        ...
        for env_index in range(self.num_parallel_envs):
            if (traj_idx := env_index_offset + env_index) < caller_locals["num_trajectories"]:
                ...
                if t <= self.hooks["episode_lengths"][traj_idx]:
                    ...
                    self.hooks["collisions_with_floor"][traj_idx, t + 1] = infos.get("collisions_with_floor", np.zeros(self.num_parallel_envs))[env_index]
                    self.hooks["passed_eye"][traj_idx, t + 1] = infos.get("passed_eye", np.zeros(self.num_parallel_envs))[env_index]
                    self.hooks["lost_grasp"][traj_idx, t + 1] = infos.get("lost_grasp", np.zeros(self.num_parallel_envs))[env_index]
                    if self.calculate_accelerations:
    ...
    def test_agent(self, agent: BaseAgent, num_trajectories: int = 10) -> dict:
        ...
        self.hooks = {
            ...
            "collisions_with_floor": np.zeros((num_trajectories, self.time_limit + 1)),
            "passed_eye": np.zeros((num_trajectories, self.time_limit + 1)),
            "lost_grasp": np.zeros((num_trajectories, self.time_limit + 1)),
        }
        if self.calculate_accelerations:
            self.hooks |= {
    ...
    def _save_episode_jsons(self, num_trajectories: int) -> None:
        try:
            ...
            for i in range(num_trajectories):
                ...
                trajectory = {
                    "tool_pos_xyz": np.nan_to_num(self.hooks["tool_pos"][i, :T + 2], nan=0.0).tolist(),
                    "collisions_with_floor": self.hooks["collisions_with_floor"][i, :T + 2].tolist(),
                    "passed_eye": self.hooks["passed_eye"][i, :T + 2].tolist(),
                    "lost_grasp": self.hooks["lost_grasp"][i, :T + 2].tolist(),
                    "rewards": np.nan_to_num(self.hooks["reward"][i, :T + 2], nan=0.0).tolist(),
                }
                if self.calculate_accelerations:
                    trajectory["rope_acceleration"] = np.nan_to_num(self.hooks["rope_accel"][i, :T + 1], nan=0.0).tolist()
                    trajectory["tool_acceleration_ptsda"] = np.nan_to_num(self.hooks["tpsd_accel"][i, :T + 1], nan=0.0).tolist()
                ...
                json_data = {
                    ...
                    "total_reward": float(np.nansum(self.hooks["reward"][i, :T + 2])),
                    "metrics": {
                        "min_tip_dist": float(self.hooks["min_tip_dist"][i]),
                        "final_tip_dist": float(self.hooks["final_tip_dist"][i]),
                        "max_rope_pts_passed": float(self.hooks["max_rope_pts_passed"][i]),
                        "final_rope_pts_passed": float(self.hooks["final_rope_pts_passed"][i]),
                        "max_rope_frac_passed": float(self.hooks["max_rope_frac_passsed"][i]),
                        "final_rope_frac_passed": float(self.hooks["final_rope_frac_passed"][i]),
                    },
