...
class BimanualTissueManipulationEnvWorkspace(BaseWorkspace):
    ...
    def render_function(self, caller_locals: Dict) -> np.ndarray:
        from sofa_env.scenes.bimanual_tissue_manipulation.bimanual_tissue_manipulation_env import ObservationType
        if self.env.observation_type == ObservationType.STATE:
            image_shape = getattr(self.env, "image_shape", (200, 200))
