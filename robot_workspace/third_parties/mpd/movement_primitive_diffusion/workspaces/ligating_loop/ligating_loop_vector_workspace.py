...
class LigatingLoopEnvVectorWorkspace(BaseVectorWorkspace):
    ...
    def check_success_hook(self, caller_locals: Dict) -> bool:
        ...
        if truncated:
            ...
            num_trajectories = len(self.hook_values["episode_lengths"])
        ...
        for env_index in range(self.num_parallel_envs):
            ...
            if trajectory_index < caller_locals["num_trajectories"]:
                ...
                if not caller_locals["done_buffer"][env_index]:
                    ...
                    self.hook_values["loop_accelerations"][trajectory_index, time_step] = info["loop_acceleration"][env_index]
                    self.hook_values["tool_positions"][trajectory_index, time_step] = info["tool_positions"][env_index]
                    self.hook_values["loop_marking_overlap"][trajectory_index, time_step] = info.get("loop_marking_overlap", np.zeros(self.num_parallel_envs))[env_index]
                    self.hook_values["loop_center_in_cavity"][trajectory_index, time_step] = float(info.get("loop_center_in_cavity", np.zeros(self.num_parallel_envs))[env_index])
                    self.hook_values["instrument_shaft_collisions"][trajectory_index, time_step] = info.get("instrument_shaft_collisions", np.zeros(self.num_parallel_envs))[env_index]
    ...
    def test_agent(self, agent: BaseAgent, num_trajectories: int = 10) -> dict:
        ...
        self.hook_values = {
            ...
            "loop_marking_overlap": np.nan * np.ones((num_trajectories, self.time_limit)),
            "loop_center_in_cavity": np.zeros((num_trajectories, self.time_limit)),
            "instrument_shaft_collisions": np.zeros((num_trajectories, self.time_limit)),
        ...
        try:
            ...
            for i in range(num_trajectories):
                ...
                json_data = {
                    ...
                    "trajectory": {
                        "tool_positions_xyz": np.nan_to_num(self.hook_values["tool_positions"][i, :T], nan=0.0).tolist(),
                        ...
                        "loop_acceleration": np.nan_to_num(self.hook_values["loop_accelerations"][i, :T], nan=0.0).tolist(),
                        "loop_marking_overlap": np.nan_to_num(self.hook_values["loop_marking_overlap"][i, :T], nan=0.0).tolist(),
                        "loop_center_in_cavity": self.hook_values["loop_center_in_cavity"][i, :T].tolist(),
                        "instrument_shaft_collisions": self.hook_values["instrument_shaft_collisions"][i, :T].tolist(),
