...
class BaseWorkspace:
    ...
    def log_video(self, frame_buffer: List[np.ndarray], fps: int, metric: str = "video") -> None:
        # Write video - always save to Hydra output directory (local)
        now = time.strftime("%Y%m%d-%H%M%S")
        
        # Always use Hydra output directory for local storage
        import hydra
        ...
        save_gif_from_array(frames=frame_buffer, fps=fps, file_path=file_path, resize_height=480)

        # Upload video to swanlab for visualization only
