...
class GraspLiftTouchEnvVectorWorkspace(BaseVectorWorkspace):
        ...
        for env_index in range(self.num_parallel_envs):
            ...
            if trajectory_index < caller_locals["num_trajectories"]:
                ...
                if not caller_locals["done_buffer"][env_index]:
                    ...
                    col_cg = info["collision_cauter_gripper"][env_index]
                    col_cb = info["collision_cauter_gallbladder"][env_index]
                    col_cl = info["collision_cauter_liver"][env_index]
                    col_gl = info["collision_gripper_liver"][env_index]
                    self.hook_values["collisions"][trajectory_index, time_step] = col_cg + col_cb + col_cl + col_gl
                    self.hook_values["collision_cauter_gallbladder"][trajectory_index, time_step] = col_cb
                    self.hook_values["collision_cauter_liver"][trajectory_index, time_step] = col_cl
                    self.hook_values["collision_gripper_liver"][trajectory_index, time_step] = col_gl
                    self.hook_values["force_on_gallbladder"][trajectory_index, time_step] = info.get("dynamic_force_on_gallbladder", np.zeros(self.num_parallel_envs))[env_index]
    ...
    def test_agent(self, agent: BaseAgent, num_trajectories: int = 10) -> dict:
        ...
        self.hook_values = {
            ...
            "force_on_gallbladder": np.nan * np.ones((num_trajectories, self.time_limit)),
            "collision_cauter_gallbladder": np.zeros((num_trajectories, self.time_limit)),
            "collision_cauter_liver": np.zeros((num_trajectories, self.time_limit)),
            "collision_gripper_liver": np.zeros((num_trajectories, self.time_limit)),
        ...
        try:
            ...
            for i in range(num_trajectories):
                ...
                json_data = {
                    ...
                    "metrics": {
                        "final_phase": float(self.hook_values["final_phase"][i]),
                    },
                    "trajectory": {
                        ...
                        "tool_acceleration": np.nan_to_num(self.hook_values["tool_accelerations"][i, :T], nan=0.0).tolist(),
                        "force_on_gallbladder": np.nan_to_num(self.hook_values["force_on_gallbladder"][i, :T], nan=0.0).tolist(),
                        "collisions_total": np.nan_to_num(self.hook_values["collisions"][i, :T], nan=0.0).tolist(),
                        "collision_cauter_gallbladder": self.hook_values["collision_cauter_gallbladder"][i, :T].tolist(),
                        "collision_cauter_liver": self.hook_values["collision_cauter_liver"][i, :T].tolist(),
                        "collision_gripper_liver": self.hook_values["collision_gripper_liver"][i, :T].tolist(),
