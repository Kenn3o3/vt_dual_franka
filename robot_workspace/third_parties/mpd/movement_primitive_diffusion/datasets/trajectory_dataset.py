...
class TrajectoryDataset(Dataset):
    ...
    def __load_trajectories(self) -> None:
        ...
        for trajectory_dir in self.trajectory_dirs:
            ...
            # For image keys, prefer a per-trajectory tensor cache. Falling back to
            # per-frame PNGs is correct but much slower for overlapping subsequences.
            for key in self.image_keys:
                ...
                is_a_npy_file = (trajectory_dir / Path(key + ".npy")).exists()
                is_a_directory = (trajectory_dir / Path(key)).is_dir()

                if is_a_npy_file:
                    images_tensor = torch.from_numpy(np.load(trajectory_dir / Path(key + ".npy"))).to(torch.uint8)
                    ...
                    raise ValueError(f"Could not find '{key}.npy', '{key}.npz', or '{key}' directory in '{trajectory_dir}'.")
