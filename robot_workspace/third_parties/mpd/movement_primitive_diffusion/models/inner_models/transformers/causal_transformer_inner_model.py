...
class CausalTransformer(torch.nn.Module):
    ...
    def forward(
        ...
        # Clip positional embedding and masks to the actual sequence length of the input.
        # This is necessary for Streaming Flow Policy which feeds a single token per ODE
        # step (seq_len=1), while the transformer was built for the full action_time_steps.
        # Without clipping, PyTorch broadcasting turns [B,1,E]+[1,T,E] into [B,T,E],
        # creating T phantom tokens from a single input token and diluting gradients by T.
        seq_len = token_embeddings.shape[1]
        pos_emb = self.position_embedding[:, :seq_len, :]
        tgt_mask = self.mask[:seq_len, :seq_len]
        mem_mask = self.memory_mask[:seq_len, :] if self.memory_mask is not None else None
        # (B, seq_len, embedding_size)
        x = self.drop(token_embeddings + pos_emb)
        # (B, seq_len, embedding_size)
        x = self.decoder(tgt=x, memory=memory, tgt_mask=tgt_mask, memory_mask=mem_mask)
