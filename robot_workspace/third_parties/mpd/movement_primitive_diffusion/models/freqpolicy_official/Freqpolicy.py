...
from movement_primitive_diffusion.models.freqpolicy_official.diffloss import DiffLoss
import torch_dct
...
class Freqpolicy(nn.Module):
    ...
    def __init__(self, trajectory_dim=26, horizon=16, n_obs_steps=2,
        ...
        # buffer_size should be n_obs_steps + 1 (for index embedding)
        self.buffer_size = n_obs_steps + 1
        core_2 =  5
        if num_iter == 1:
            self.core = [0]
        else:
    ...
    def sample_orders(self, bsz, device):
        ...
        for _ in range(bsz):
            ...
            orders.append(order)
        orders = torch.as_tensor(np.array(orders), device=device).long()
        return orders
