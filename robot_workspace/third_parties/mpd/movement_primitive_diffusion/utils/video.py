...
def save_gif_from_array(frames: List[np.ndarray], fps: Union[float, int], file_path: Union[str, Path], resize_height: Optional[int] = None) -> None:
    """Creates a GIF from an array of frames and saves it to a file.

    Parameters:
        frames (List[np.ndarray]): List of frames to be used in the GIF.
        fps (Union[float, int]): Frames per second for the output GIF.
        file_path (Union[str, Path]): Path to the output GIF file (e.g., 'output.gif').
        resize_height (Optional[int]): If specified, the frames will be resized to this height.
    """
    ...
    video_clip.write_gif(str(file_path), fps=fps)
