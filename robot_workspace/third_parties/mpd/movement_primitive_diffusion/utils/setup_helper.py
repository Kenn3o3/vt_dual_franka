...
# DEPRECATED: This module has been refactored into movement_primitive_diffusion.training
# Please update your imports to use: from movement_primitive_diffusion.training import ...
...
from movement_primitive_diffusion.training import (
    get_dataloaders,
...
from movement_primitive_diffusion.training.config_manager import setup_agent_and_workspace

log = logging.getLogger(__name__)


# All functions below are re-exported from the training module for backward compatibility

def _get_dataloaders_for_fixed_split_deprecated(cfg: DictConfig) -> Tuple[DataLoader, DataLoader]:
...
def setup_swanlab_metrics(workspace_result_keys: List[str], performance_metric: str) -> None:
    """Helper function to set up the metrics for swanlab.
...
def setup_swanlab_test_metrics(workspace_result_keys: List[str]) -> None:
    """Helper function to set up the metrics for swanlab in the test scripts.
...
def parse_swanlab_to_hydra_config(swanlab_config: DictConfig) -> DictConfig:
    """Parses a swanlab config to a hydra config.

    Args:
        swanlab_config (DictConfig): The swanlab config. At the first level, a swanlab config
        contains a key "swanlab_version" and a key for each hyperparameter. Each hyperparameter
        contains a description and a value. We only need the value.

    Returns:
        hydra_config (DictConfig): The hydra config. Removes the "swanlab_version" key and
        only keeps the hyperparameters and their values.
    """
    hydra_config = OmegaConf.create()

    for key, value in swanlab_config.items():
        if key == "swanlab_version":
            continue
