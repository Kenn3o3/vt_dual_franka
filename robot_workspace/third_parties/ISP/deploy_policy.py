...
from isp.common.univtac_util import canonicalize_gripper_qpos
...
# Map from ISP obs key → simulator observation path
# Each value is (top_level_key, sensor_name, data_key)
_OBS_SOURCE = {
    "robot0_eye_in_hand_image": ("observation", "wrist", "rgb"),
    "robot0_tactile_left_image": ("tactile", "left_tactile", "rgb"),
    "robot0_tactile_right_image": ("tactile", "right_tactile", "rgb"),
}
...
class Policy(BasePolicy):
    def __init__(self, args):
        """Initialize ISP policy for deployment (SO3 / SO2 / SO2-tact)."""
        task_name = args["task_name"]
        ep_num = os.environ.get("EP_NUM", "100")
        ckpt_path = os.environ.get("ISP_CKPT_PATH")
        if not ckpt_path:
            raise ValueError(
                "ISP deployment now requires ISP_CKPT_PATH from the clean orchestration layer."
            )
        ...
        self.device = torch.device(
            os.environ.get("ISP_DEVICE", "cuda" if torch.cuda.is_available() else "cpu")
        )
        self.model.eval()
        ...
        self.execution_horizon = int(
            os.environ.get("ISP_EXEC_HORIZON", "1")
        )
        self.execution_horizon = max(1, min(self.execution_horizon, self.n_action_steps))
        self.postprocessing = os.environ.get("ISP_POSTPROCESSING", "normal").lower()
        if self.postprocessing not in {"normal", "temporal_agg"}:
            raise ValueError(
                f"Unsupported ISP_POSTPROCESSING={self.postprocessing}. "
                "Expected one of: normal, temporal_agg."
            )
        self.temporal_agg_k = float(os.environ.get("ISP_TEMPORAL_AGG_K", "0.01"))
        if self.postprocessing == "temporal_agg":
            self.execution_horizon = 1
        self.obs_clamp_mode = os.environ.get("ISP_OBS_CLAMP_MODE", "off").lower()
        self.max_inferences = int(os.environ.get("ISP_MAX_INFERENCES", "20"))
        self.action_type = os.environ.get("ISP_ACTION_TYPE", "ee_servo").lower()
        if self.action_type not in {"ee", "ee_servo"}:
            raise ValueError(f"Unsupported ISP_ACTION_TYPE={self.action_type}. Expected one of: ee, ee_servo")
        ...
        # NOTE: historical ISP deploy resized every RGB observation to 84x84 here.
        ...
        print(
            f"ISP policy ({variant}) loaded from {ckpt_path}\n"
            ...
            f"  rgb_keys={self.obs_rgb_keys}, lowdim_keys={self.obs_lowdim_keys}"
        )
        ...
        if self.execution_horizon > 1:
            print("  [WARN] ISP is running open-loop chunk execution; horizon=1 is recommended.")
    ...
    def _get_image(self, observation, isp_key):
        ...
        """
        source = _OBS_SOURCE.get(isp_key)
        if source is None:
            raise KeyError(f"No simulator source mapping for ISP obs key '{isp_key}'")
        ...
        if isp_key == "robot0_eye_in_hand_image":
        ...
        target_hw = self.obs_rgb_shapes[isp_key]
        # NOTE: historical ISP deploy used the single fixed transform below.
        # return self.img_transform(img_t)  # (3, 84, 84)
        if tuple(img_t.shape[-2:]) != target_hw:
            img_t = transforms.Resize(target_hw)(img_t)
        return img_t

    def _get_raw_image(self, observation, isp_key):
        source = _OBS_SOURCE.get(isp_key)
        if source is None:
            raise KeyError(f"No simulator source mapping for ISP obs key '{isp_key}'")
        top, sensor, data_key = source
    ...
    def encode_obs(self, observation):
        """Convert simulator observation to a single-frame ISP obs dict."""
        obs = {}
        ...
        for key in self.obs_lowdim_keys:
            if key == "robot0_eef_pos":
                ...
                # Pose.totensor() is wxyz; ISP normalizer trained on wxyz as-is.
                ...
                raise KeyError(f"Unknown ISP lowdim obs key: '{key}'")
        ...
        if self.postprocessing == "temporal_agg":
            if self.max_inferences > 0 and self._inference_count >= self.max_inferences:
                ...
                if hasattr(task, 'request_stop'):
                    task.request_stop(f'ISP reached max inference count: {self.max_inferences}')
                return
