...
from isp.common.univtac_util import (
    canonicalize_gripper_qpos,
    compute_ws_center,
    gripper_scalar_from_qpos,
)
from isp.dataset.base_dataset import BaseImageDataset
from isp.model.common.normalizer import LinearNormalizer, SingleFieldLinearNormalizer
...
def _resolve_h5_source_path(h5_file: h5py.File, key: str) -> str:
    key_to_candidates = {
    ...
    if candidates is None:
        raise ValueError(f"Unsupported RGB key in shape_meta: {key}")
...
def _episode_to_arrays(
    ...
    with h5py.File(str(episode_path), "r") as f:
        ee = f["embodiment/ee"][:]  # (T, 7), pos + quat(wxyz)
        joint = f["embodiment/joint"][:]  # (T, 9)
        if ee.shape[0] < 2:
            raise ValueError(f"Episode {episode_path} has <2 frames")

        obs_ee = ee[:-1]
        act_ee = ee[1:]
        obs_joint = joint[:-1]
        act_joint = joint[1:]
        steps = obs_ee.shape[0]
        ...
        action_gripper = gripper_scalar_from_qpos(act_joint[:, 7:9]).astype(np.float32) # take mean
        ...
        for key in lowdim_keys:
            if key == "robot0_eef_pos":
                arrays[key] = obs_ee[:, :3].astype(np.float32)
            elif key == "robot0_eef_quat":
                arrays[key] = obs_ee[:, 3:7].astype(np.float32)
            elif key == "robot0_gripper_qpos":
                arrays[key] = canonicalize_gripper_qpos(obs_joint[:, 7:9]).astype(np.float32) # normailzed
            else:
        ...
        for key in rgb_keys:
            ...
            source_path = _resolve_h5_source_path(f, key)
            center_crop_square = (
            ...
            arrays[key] = decoded_cache[cache_key][:-1]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_cache:
            ...
            cache_name = (
                f"univtac_{task_name}_{cache_split}_demo{n_demo}_"
        ...
        if n_obs_steps is not None:
            for key in rgb_keys + lowdim_keys:
                ...
                print("key: ", key)
                print(f"[UniVTACDataset] Using first {n_obs_steps} steps of '{key}' for obs and normalizer")
