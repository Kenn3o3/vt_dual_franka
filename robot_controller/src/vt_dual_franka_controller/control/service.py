...
from vt_dual_franka_shared.interpolation import PoseTrajectoryInterpolator, pose_distance
...
from vt_dual_franka_shared.pose_math import pose7d_to_pose6d
...
class ControllerService:
    ...
    def queue_tcp_command(self, command: TcpTargetCommand) -> None:
        ...
        target_time = float(command.target_monotonic_time) if command.target_monotonic_time is not None else time.monotonic() + target_duration_sec
    ...
    def get_state(self) -> ControllerState:
        ...
        state = state.model_copy(update={"arm_id": self.settings.arm_id})
        with self._state_lock:
