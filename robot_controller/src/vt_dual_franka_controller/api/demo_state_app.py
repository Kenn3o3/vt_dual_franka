...
def create_demo_state_app(service) -> FastAPI:
    app = FastAPI(title="VT Dual Franka Demo State", version="0.1.0")
...
def _ensure_left(robot_side: str) -> None:
    if robot_side != "left":
        raise HTTPException(status_code=400, detail="Only the left arm is supported in VT Dual Franka v1")
